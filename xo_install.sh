...
if [ -d "$xowebDIR" ]; then
    ...
    tar -zcf "$WORKDIR"/xo-web-$(date +%Y-%m-%d-%X).tar.gz "$xowebDIR"
    printf '%s\n' "Removing old ($xowebDIR)"
    rm -rf "$xowebDIR"
fi
if [ -d "$xoserverDIR" ]; then
    printf '%s\n' "Creating backup of ($xoserverDIR)"
    tar -zcf "$WORKDIR"/xo-server-$(date +%Y-%m-%d-%X).tar.gz "$xoserverDIR"
    printf '%s\n' "Removing old ($xoserverDIR)"
    rm -rf "$xoserverDIR"
fi

apt-get update
apt-get install --yes nfs-common git curl npm build-essential redis-server libpng12-dev git python-minimal
cd "$WORKDIR"
